...
void tool_change(const uint8_t tmp_extruder, const float fr_mm_s/*=0.0*/, bool no_move/*=false*/)
{
		...
		#if HOTENDS > 1
			SERIAL_ECHOPGM("tool_change Need change Extruder axis! ");
			SERIAL_ECHOLNPAIR("tmp_extruder = ",(int)tmp_extruder);
...
inline void gcode_T(uint8_t tmp_extruder) {
  ...
  #elif HOTENDS > 1
	SERIAL_ECHOPGM("gcode_T Need change Extruder axis! ");
	SERIAL_ECHOLNPAIR("tmp_extruder = ",(int)tmp_extruder);
    tool_change(
      tmp_extruder,
...
//			card.removePowerOffFile();   
			...
			{ 
				...
				enable_Z();
				SERIAL_PROTOCOLLN("current_position(X,Y,Z,E,F,T0,T1,..T4,B): ");
...
//       sprintf_P(power_off_commands[3], PSTR("T%s"), str_T);          
 ...
 */ 
  ...
  pinMode(CHECK_MATWEIAL, INPUT);
    digitalWrite(CHECK_MATWEIAL, LOW); // turn it off
/*
  pinMode(CHECK_MATWEIAL1, INPUT);
    digitalWrite(CHECK_MATWEIAL1, LOW); // turn it off
*/
